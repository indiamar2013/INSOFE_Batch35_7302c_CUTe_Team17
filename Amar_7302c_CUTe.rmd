...
fin_train <- fin_data[rownames(y1_data) <= as.Date('2014-12-31', format = '%Y-%m-%d'), ]

fin_test <- fin_data[rownames(y1_data) > as.Date('2014-12-31', format = '%Y-%m-%d'), ]
...
library(RANN)
set.seed(1234)
preproc_preds <- preProcess(x = subset(fin_train, select = -c(y1, y2)), method = c("center", "scale", "knnImpute"))
...
arima1 <- Arima(y = y1ts, order = c(5,0,0), xreg = subset(y1_train, select= c(d_0, d_1, d_2, d_3, d_4, f_0, f_5, f_7, f_8, f_9, f_11, f_13, f_14, f_15, f_16, f_18,  f_19, f_20 , f_22 , f_23 , f_24 , f_29 , f_30 , f_32 , f_33 , f_34 , f_36 , f_37 , f_39 , f_40 , f_41 , f_42 , f_43 , f_44 , f_46 , f_48 , f_49 , f_50 , f_51 , f_52 , f_53 , f_54 , f_56 , f_58 , f_59 , f_60 , f_62 , t_0 , t_2 , t_3 , t_6 , t_7 , t_11 , t_12 , t_14 , t_17 , t_18 , t_19 , t_21 , t_24 , t_25 , t_27 , t_29 , t_32 , t_33 , t_34 , t_35 , t_36 , t_37 , t_38 , t_39 , t_40 , t_43 , t_44)), include.drift = TRUE)

arima1

fin_arima1 <- auto.arima(y1ts)
fin_arima1
```
