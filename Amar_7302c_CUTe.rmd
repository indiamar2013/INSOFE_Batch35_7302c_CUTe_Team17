...
nacolstoremove <- colswithna[colswithna > nrow(fin_data)*0.25]
nacolstoremove
```
...
base_linreg <- lm(100*(1+y1) ~ ., data = y1_train)
...
```{r}
library(MASS)
aicoptions <- stepAIC(new_lm_mdl, direction = "both", trace = FALSE)
summary(aicoptions)
```
...
lm(formula = 100*(1+y1) ~ d_0 + d_1 + d_4 + f_16 + f_18 + f_20 + f_21 + 
...
print('Error metrics for Train data')
print(regr.eval(aic_preds, new_y1_train$newy1))
print("")
print('Error metrics for Test data')
print(regr.eval(aic_preds, setdailyprice(y1_test$y1)))
